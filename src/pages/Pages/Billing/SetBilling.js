...
const SetBilling = props => {
    ...
    () => [
      ...
      },
      {
        ...
        Footer: ({ table }) => {
          ...
          let sum_rate = 0;
          rows.map((e, i) => {
            if (!isNaN(e.original.rate)) sum_rate += e.original.rate;
          });
          let f = footer;
          f[3] = sum_rate;
          setFooter(f);
      ...
      },
      {
        ...
        Footer: ({ table }) => {
          ...
          let sum_qty = 0;
          rows.map((e, i) => {
            if (!isNaN(e.original.qty)) sum_qty += e.original.qty;
          });
          let f = footer;
          f[4] = sum_qty;
          setFooter(f);
  ...
  const addItem = async (billingData, footer, toParty, date) => {
    ...
    if (props.editingData !== null) {
      await api_edit_billing(payload)
        .then(res => {
          if (res.success) {
            console.log(res);
          ...
          <Grid.Col span={6}>
            <Flex h={"100%"} direction={"column"} align={"center"} justify={"space-evenly"}>
              <Button
                ...
                onClick={() => {
                  let billing_items = billingData?.filter(
                    (e, i) => e.amount > 0 && e.supplier_party
                  );
